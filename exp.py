...
def main():
    ...
    ap.add_argument('--class_dir', default="F:\datasets\第一批胰腺\胰腺数据\分类\黏液性", type=Path,
                    help='分类/浆液性 文件夹路径（包含图片）')
    ap.add_argument('--ann_dir', default="F:\datasets\第一批胰腺\胰腺数据\检测\VOCdevkit\VOC2007\Annotations", type=Path,
                    help='VOC Annotations 原标注文件夹（XML）')
    ap.add_argument('--out_dir', default="F:\datasets\第一批胰腺\胰腺数据\检测\VOCdevkit\VOC2007\Annotations_SCN", type=Path,
                    help='新的标注输出文件夹')
    ap.add_argument('--label', default='MCN', help='目标类别名，默认 SCN')
    ap.add_argument('--copy_others', action='store_true',
                    help='把未命中的其它 XML 也原样拷贝到 out_dir')
    args = ap.parse_args()
