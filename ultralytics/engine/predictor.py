...
class BasePredictor:
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            #     self.model.warmup(
            #         imgsz=(1 if self.model.pt or self.model.triton else self.dataset.bs, self.model.ch * 2, *self.imgsz)
            #     )
            ...
            for self.batch in self.dataset:
                ...
                im0s_ir = cv2.imread(paths[0].replace('images', 'images-ir'))
                im0s = [cv2.merge((im0s_ir, im0s[0]))]
                # Preprocess
                with profilers[0]:
