...
def soft_nms(
    ...
    """boxes: [N,4] (xyxy) scores: [N] method: "linear" | "gaussian" 返回：保留下来的原始索引（LongTensor），按选择顺序排列.
    """
    if boxes.numel() == 0:
    ...
    while scores.numel():
        # 选当前分数最高的
        max_i = torch.argmax(scores)
        max_box = boxes[max_i].unsqueeze(0)  # [1,4]
        max_idx = idxs[max_i].item()
        keep.append(max_idx)
        ...
        if method == "linear":
            decay = torch.ones_like(rest_scores)
            m = ious > iou_threshold
            decay[m] = 1.0 - ious[m]
            rest_scores = rest_scores * decay
        elif method == "gaussian":
            # 论文中的高斯衰减：exp(-(iou^2)/sigma)
            rest_scores = rest_scores * torch.exp(-(ious * ious) / sigma)
        else:
            # 等价于标准 NMS（硬抑制）
            rest_scores = rest_scores.masked_fill(ious > iou_threshold, 0.0)

        # 丢掉过低分数，继续循环
        keep_mask = rest_scores > score_threshold
