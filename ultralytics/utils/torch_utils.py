...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    # flops = get_flops(model, [1280,416])  # imgsz may be int or list, i.e. imgsz=640 or imgsz=[640, 320]
    # flops = get_flops(model, [1280,384])  # imgsz may be int or list, i.e. imgsz=640 or imgsz=[640, 320]
    # flops = get_flops(model, [1664,544])  # imgsz may be int or list, i.e. imgsz=640 or imgsz=[640, 320]
    # flops = get_flops(model, [480,1632])  # imgsz may be int or list, i.e. imgsz=640 or imgsz=[640, 320]
    # flops = get_flops(model, [1600,512])  # imgsz may be int or list, i.e. imgsz=640 or imgsz=[640, 320]
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        im = torch.empty((1, 6, *imgsz), device=p.device)  # input image in BCHW format
