...
from .mafyolo import UniRepLKNetBlock
...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        # )
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 1), UniRepLKNetBlock(c2, 5), Conv(c2, c2, 1), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
            )
        self.cv3 = (
...
class Detect_Aux(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor]) -> Union[List[torch.Tensor], Tuple]:
        ...
        for i in range(self.nl // 2, self.nl):
            j = i - self.nl // 2
            x[i] = torch.cat((self.cv2_aux[j](x[i]), self.cv3_aux[j](x[i])), 1)
        if self.training:  # Training path
            return x
        x = x[0:3]
        y = self._inference(x)
