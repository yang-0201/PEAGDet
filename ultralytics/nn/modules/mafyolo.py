...
class TFCF(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h=8, block_exp=4, n_layer=1, attn_pdrop=0.1,
                 resid_pdrop=0.1):
        super(TFCF, self).__init__()
        ...
        self.crosstransformer = nn.Sequential(
            *[CrossTransformerBlock(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop) for i in
              range(n_layer)])
...
class CrossTransformerBlock(nn.Module):
    def __init__(self, d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop, loops_num=1):
        ...
        self.mlp_N = nn.Sequential(nn.Linear(d_model, block_exp * d_model),
        ...
        self.ffn_vis = nn.Sequential(
            Conv_trans(d_model, d_model*2, 1),
            Conv_trans(d_model*2, d_model, 1, act=False)
        )
        self.mlp_A = nn.Sequential(nn.Linear(d_model, block_exp * d_model),
                                    # nn.SiLU(),  # changed from GELU
                                    nn.GELU(),  # changed from GELU
                                    nn.Linear(block_exp * d_model, d_model),
                                    nn.Dropout(resid_pdrop),
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        q_N = self.que_proj_N(N_fea_flat).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, nq, d_k)
        k_N = self.key_proj_N(N_fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, d_k, nk) K^T
        v_N = self.val_proj_N(N_fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)

        A_fea_flat = self.LN2(A_fea_flat)
        q_A = self.que_proj_A(A_fea_flat).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, nq, d_k)
        k_A = self.key_proj_A(A_fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, d_k, nk) K^T
        v_A = self.val_proj_A(A_fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)
        ...
        # output
        out_N = torch.matmul(att_N, v_N).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_N = self.resid_drop(self.out_proj_N(out_N)) # (b_s, nq, d_model)
        out_A = torch.matmul(att_A, v_A).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
...
class RepHEA(nn.Module):
    def __init__(self, in_channels, out_channels, width=3, depth=1, depth_expansion=2, kersize=5, shortcut=True,
        ...
        self.directional_blockH = UniRepSingleAxisConvBlock(dim=c__, kernel_sizes=[5, 7, 9, 11, 13], is_vertical=False, deploy=False)
        # self.Hsilu = nn.SiLU()
        self.directional_blockV = UniRepSingleAxisConvBlock(dim=c__, kernel_sizes=[5, 7, 9, 11, 13], is_vertical=True, deploy=False)
        # self.Vsilu = nn.SiLU()
        self.golbel =  DepthBottleneckUniv3(self.c_, self.c_, shortcut, kersize, depth_expansion, small_kersize, use_depthwise)
...
class DepthBottleneckUniv3(nn.Module):
    def __init__(self,
                 ...
                 small_kersize=3,
                 use_depthwise=True):
        super(DepthBottleneckUniv3, self).__init__()
...
class UniRepSingleAxisConvBlock(nn.Module):
    def __init__(self,
                 ...
                 attempt_use_lk_impl=True):
        """
        与UniRepLKNetBlock类似，将多个单轴卷积并行，然后可在reparameterize时合并。
        kernel_sizes例如：[5,7,9]表示并行三个卷积核。
        is_vertical=True表示 1xK 纵向卷积，否则为 Kx1 横向卷积。
        """
        super().__init__()
        if deploy:
            print('------------------------------- Note: deploy mode')
        ...
        self.single_axis_block = SingleAxisReparamBlock(dim, kernel_sizes, is_vertical=is_vertical, deploy=deploy,
                                                        attempt_use_lk_impl=attempt_use_lk_impl)
...
class SingleAxisReparamBlock(nn.Module):
    """
    与 DilatedReparamBlock 类似，但针对单轴并行卷积核的重参数化。
    例如：并行几个1x5, 1x7, 1x9的卷积，然后合并为一个大的1x9卷积。
    """
    def __init__(self, channels, kernel_sizes, is_vertical=True, deploy=False, attempt_use_lk_impl=True):
        ...
        if self.is_vertical:
            # 纵向 1 x max_kernel
            self.lk_origin = get_conv2d_uni(channels, channels, kernel_size=(1, max_kernel), stride=1,
                                            padding=(0, max_kernel//2), dilation=1, groups=channels, bias=deploy,
                                            attempt_use_lk_impl=attempt_use_lk_impl)
        else:
            # 横向 max_kernel x 1
            self.lk_origin = get_conv2d_uni(channels, channels, kernel_size=(max_kernel, 1), stride=1,
                                            padding=(max_kernel//2, 0), dilation=1, groups=channels, bias=deploy,
                                            attempt_use_lk_impl=attempt_use_lk_impl)
        ...
        if not deploy:
            ...
            for k in kernel_sizes:
                if k == max_kernel:
                    ...
                    continue
                if self.is_vertical:
                    conv = nn.Conv2d(channels, channels, kernel_size=(1, k), stride=1,
                                     padding=(0, k//2), dilation=1, groups=channels, bias=False)
                else:
                    conv = nn.Conv2d(channels, channels, kernel_size=(k, 1), stride=1,
                                     padding=(k//2, 0), dilation=1, groups=channels, bias=False)
                bn = get_bn(channels)
    ...
    def merge_single_axis_branches(self):
        ...
        if hasattr(self, 'origin_bn'):
            ...
            out_channels, in_channels, h, w = origin_k.size()
            # h和w中有一个维度应为1（单轴卷积）
            ...
            merged_conv = get_conv2d_uni(out_channels, out_channels, final_kernel_size,
                                         stride=1, padding=final_padding, dilation=1,
...
def center_merge_kernel(large_kernel, small_kernel):
    """
    将一个较小的核居中对齐到large_kernel大小的核上，并相加。
    假设 large_kernel 和 small_kernel 都是 (C, 1, H, W) 或 (C, C, H, W)
    其中 H 和 W 是卷积核的高和宽，C 是通道数
    """
    C = large_kernel.size(0)  # 获取通道数
