...
class BaseModel(torch.nn.Module):
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                ...
                if isinstance(m, UniRepSingleAxisConvBlock):
                    m.reparameterize()
                    LOGGER.info("Switch model to UniRepSingleAxisConvBlock")
    ...
    def load(self, weights, verbose=True):
        ...
        print("all_weights:", len(csd))
        updated_csd = intersect_dicts(csd, self.state_dict())  # intersect
        print("load_weights:", len(updated_csd))
...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        # return E2EDetectLoss(self) if getattr(self, "end2end", False) else v8DetectionLoss_Aux(self)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, Detect_Aux}:
                m.legacy = legacy
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect, Detect_Aux)):
                return "detect"
