...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=6):
        ...
        assert n in {4, 6, 9}, "grid must be equal to 4 or 9."
    ...
    def _mosaic6(self, labels):
        """Create a 3x2 image mosaic (two columns, three rows) from six images.

        Layout order (placed on a 3s x 3s canvas, then center-cropped to 2s x 2s):
            0: left-middle  (from labels)
            1: right-middle (from mix_labels[0])
            2: left-top     (from mix_labels[1])
            3: right-top    (from mix_labels[2])
            4: left-bottom  (from mix_labels[3])
            5: right-bottom (from mix_labels[4])
        """
        mosaic_labels = []
        s = self.imgsz

        # base canvas: 3s x 3s, keep center-crop behavior same as _mosaic3/_mosaic9
        C = labels["img"].shape[2]
        img6 = np.full((s * 3, s * 3, C), 114, dtype=np.uint8)

        # column vertical anchors (for stacking top/mid/bottom without overlap)
        col_edges = {
            "L": {"top": s, "bottom": None},  # y = s is vertical center line
            "R": {"top": s, "bottom": None},
        }

        def place_patch(img, h, w, col, pos):
            """Paste one tile into img6 and compute padding offsets for labels. col: "L" or "R"; pos: "mid" | "top" |
            "bottom".
            """
            if col == "L":
                ...
                if pos == "mid":
                    y1a = s
                    y2a = min(s + h, 3 * s)
                    col_edges["L"]["bottom"] = y2a
                elif pos == "top":
                    y2a = col_edges["L"]["top"]
                    y1a = max(y2a - h, 0)
                    col_edges["L"]["top"] = y1a
                else:  # bottom
                    y1a = col_edges["L"]["bottom"]
                    y2a = min(y1a + h, 3 * s)
                    col_edges["L"]["bottom"] = y2a

                # source crop in the small image
                x1b = w - (x2a - x1a)
                y1b = 0 if pos != "top" else h - (y2a - y1a)
                x2b = w
                y2b = min(h, y1b + (y2a - y1a))

            else:
                # Right column starts at x = s
                x1a = s
                x2a = min(s + w, 3 * s)
                if pos == "mid":
                    y1a = s
                    y2a = min(s + h, 3 * s)
                    col_edges["R"]["bottom"] = y2a
                elif pos == "top":
                    y2a = col_edges["R"]["top"]
                    y1a = max(y2a - h, 0)
                    col_edges["R"]["top"] = y1a
                else:  # bottom
                    y1a = col_edges["R"]["bottom"]
                    y2a = min(y1a + h, 3 * s)
                    col_edges["R"]["bottom"] = y2a
                ...
                x1b = 0
                y1b = 0 if pos != "top" else h - (y2a - y1a)
                x2b = min(w, x1b + (x2a - x1a))
                y2b = min(h, y1b + (y2a - y1a))

            # paste
            img6[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]
        ...
        for i, (col, pos) in enumerate(order):
            ...
            padw, padh = place_patch(img, h, w, col, pos)
            # add border offset since we will center-crop from 3s->2s later
...
class VerticalConcat(BaseMixTransform):
    """Apply vertical concatenation (stacking) augmentation to image datasets.

    This class implements a vertical image concatenation technique where two images are stacked vertically, and their
    corresponding labels (YOLO format) are combined accordingly.

    Attributes:
        dataset (Any): The dataset to which vertical concatenation will be applied.
        pre_transform (Callable | None): Optional transform to apply before concatenation.
        p (float): Probability of applying vertical concatenation.

    Methods:
        _mix_transform: Apply vertical concatenation to the input image and labels.

    Examples:
        >>> from ultralytics.data.augment import VerticalConcat
        >>> dataset = YourDataset(...)  # Your image dataset
        >>> concat = VerticalConcat(dataset, p=0.5)
        >>> augmented_labels = concat(original_labels)
    """

    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        """Initialize the VerticalConcat augmentation object.

        Args:
            dataset (Any): The dataset to which vertical concatenation will be applied.
            pre_transform (Callable | None): Optional transform to apply before concatenation.
            p (float): Probability of applying vertical concatenation. Must be in the range [0, 1].

        Examples:
            >>> from ultralytics.data.dataset import YOLODataset
            >>> dataset = YOLODataset("path/to/data.yaml")
            >>> concat = VerticalConcat(dataset, pre_transform=None, p=0.5)
        """
        super().__init__(dataset=dataset, pre_transform=pre_transform, p=p)

    def _mix_transform(self, labels):
        """Apply vertical concatenation to the input image and labels.

        Args:
            labels (dict): A dictionary containing the original image and label information.

        Returns:
            (dict): A dictionary containing the vertically concatenated image and combined labels.

        Examples:
            >>> concat = VerticalConcat(dataset)
            >>> concat_labels = concat._mix_transform(labels)
        """
        # Get the second image and its labels
        ...
        # Adjust bounding box coordinates for the second image
        h1, _w1 = img1.shape[:2]
        h2, _w2 = img2.shape[:2]

        # Shift YOLO bbox coordinates of the second image down by h1 (normalized)
        instances2 = labels2["instances"]
        if len(instances2.bboxes) > 0:
            instances2.bboxes[:, 1] += h1 / h2  # y_center shift (normalized)
            instances2.bboxes[:, 3] *= h1 / h2  # height scaling (normalized)
...
class CopyPaste(BaseMixTransform):
    ...
    #     """
    #     Copy-Paste augmentation: paste objects from labels2 into labels1.
    #
    #     Args
    #         labels1 (dict): target image/labels.
    #         labels2 (dict, optional): source image/labels. If None, use a flipped copy of labels1.
    #     """
    #     if labels2 is None:
    #         labels2 = {}
    #
    ...
    #
    #     # 获取 source instances
    #     src_instances = labels2.pop("instances", None)
    #     if src_instances is None:
    #         src_instances = deepcopy(instances)
    #         src_instances.fliplr(w)
    #
    #     ioa = bbox_ioa(src_instances.bboxes, instances.bboxes)  # (N, M)
    #     keep_idx = np.nonzero((ioa < 0.30).all(1))[0]  # (N,)
    #     keep_idx = keep_idx[np.argsort(ioa.max(1)[keep_idx])]  # 按最大 IOA 升序
    #     keep_idx = keep_idx[: round(self.p * len(keep_idx))]
    #
    #     if len(keep_idx) == 0:  # 无可用实例，直接返回
    #         labels1["img"] = img
    #         labels1["cls"] = cls
    ...
    #
    #     # 合并类别和实例
    #     cls = np.concatenate((cls, labels2.get("cls", cls)[keep_idx]), axis=0)
    #     instances = Instances.concatenate((instances, src_instances[keep_idx]), axis=0)
    #
    #     # 生成 mask（RGB 与 IR 共用同一轮廓）
    #     mask = np.zeros((h, w), dtype=np.uint8)
    #     for seg in src_instances.segments[keep_idx]:
    #         cv2.drawContours(mask, [seg.astype(np.int32)], -1, 1, cv2.FILLED)
    #
    #     # 获取 source 图像
    #     src_img = labels2.get("img", cv2.flip(img, 1))
    #     if src_img.ndim == 2:
    #         src_img = src_img[..., None]
    #
    #     # 一次性赋值
    #     img[mask.astype(bool)] = src_img[mask.astype(bool)]
    #
    #     # 回填结果
    #     labels1["img"] = img
    #     labels1["cls"] = cls
    ...
    def _transform(self, labels1, labels2={}):
        ...
        # ---------- 计算 IOA 并筛选 ----------
        ioa = bbox_ioa(instances2.bboxes, instances.bboxes)  # (N, M)
        indexes = np.nonzero((ioa < 0.30).all(1))[0]  # (N,)
        indexes = indexes[np.argsort(ioa.max(1)[indexes])]  # sort by overlap
        num_keep = round(self.p * len(indexes))

        # ---------- 单通道掩码 ----------
        mask2d = np.zeros((h, w), dtype=np.uint8)  # (H, W)

        for j in indexes[:num_keep]:
            # 追加类别与实例
        ...
        if result.ndim == 2:  # 灰度情况下保持第三维
            result = result[..., None]

        # ---------- 一次性更新 3/6 通道 ----------
        m = mask2d.astype(bool)  # (H, W)
        im[m] = result[m]  # Copy-Paste
        # im_new = np.zeros_like(im, np.uint8)
        # im_new[m] = 1  # 可选：调试 / 记录掩码
    ...
    #     mask_rgb = np.zeros_like(im_new[:, :, :3])
    #     # mask_ir = np.zeros_like(im_new[:, :, 3:])
    #
    #     for j in indexes[: round(self.p * n)]:
    #         cls = np.concatenate((cls, labels2.get("cls", cls)[[j]]), axis=0)
    #         instances = Instances.concatenate((instances, instances2[[j]]), axis=0)
    #         cv2.drawContours(mask_rgb, instances2.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
    #         # cv2.drawContours(mask_ir.copy(), instances2.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
    #
    #         # cv2.drawContours(im_new, instances2.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)
    #     mask_ir = mask_rgb
    #     im_new[:, :, :3][mask_rgb.astype(bool)] = 1
    #     im_new[:, :, 3:][mask_ir.astype(bool)] = 1
